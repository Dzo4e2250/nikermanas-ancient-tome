-- Najprej posodobi CHECK constraintse
...
-- Posodobi service_id v sales_funnel_questions tabeli za skupinske storitve
UPDATE public.sales_funnel_questions 
SET service_id = (
  SELECT id FROM public.services WHERE type IN ('group_live', 'group_online') LIMIT 1
)
WHERE service_id IN (
  SELECT id FROM public.services WHERE type IN ('group_live', 'group_online') OFFSET 1
);
...
-- Izbriši podvojene skupinske storitve (ohrani prvi zapis)
DELETE FROM public.services 
WHERE id IN (
  SELECT id FROM public.services WHERE type = 'group' OFFSET 1
);

-- Posodobi therapist_availability tabelo
